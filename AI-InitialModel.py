...
df = pd.read_excel("227Dataset.xlsx")

ColumnsToFix = ['lwrite','lread']
...
Terminals = [f"X{i}" for i in range(X.shape[1])] + [str(round(random.uniform(-1, 1), 2)) for _ in range(5)]
    ...
    def _evaluate_node(self, node, inputs):
        ...
        try:
            if node.data == "+":
                ...
                return left_val / (right_val + 1e-6) #Preventing division by zero
            elif node.data == "sin":
                return math.sin(left_val)
            elif node.data == "log":
                return math.log(abs(left_val) + 1e-16) #Preventing log(0)
            elif node.data == "exp":
                return math.exp(min(left_val, 50)) #Prevent Overflow
        except Exception:
            return 0 #Return safe fallback value
...
for generation in range(Generations):
    fitnesses = [calculate_tree_mse(ind, X_train, y_train) for ind in population]
    best_fitness = min(fitnesses)
    print(f"Generation {generation + 1}, Best MSE: {best_fitness:.4f}")
    ...
    while len(new_population) < PopulationSize:
        parent1 = tournament_selection(population, X_train, y_train)
        parent2 = tournament_selection(population, X_train, y_train)
...
best_tree = min(population, key=lambda t: calculate_tree_mse(t, X_train, y_train))
with open("best_tree.pk5","wb") as f: #Save the best tree as vest_tree.pk5 using pickle
    p.dump(best_tree, f)
test_mse = calculate_tree_mse(best_tree, X_test, y_test)
